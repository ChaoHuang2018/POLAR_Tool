...
int main(int argc, char *argvs[])
{
	...
	Expression<Real> deriv_x0("x2^3-x1", vars); // theta_r = 0
	Expression<Real> deriv_x1("x2", vars);
	Expression<Real> deriv_x2("u", vars);
	...
	*/
	ODE<Real> dynamics({"x2^3-x1","x2","u", "0"}, vars);
	 ...
	 */
	Interval init_x0(stod(argv[4]), stod(argv[5])), init_x1(stod(argv[6]), stod(argv[7])), init_x2(stod(argv[8]), stod(argv[9])), init_u(stod(argv[10]), stod(argv[11])); // w=0.05
	...
	else
	{
		...
		cout << "Terminated due to too large overestimation." << endl;
		exit(1);
	}
