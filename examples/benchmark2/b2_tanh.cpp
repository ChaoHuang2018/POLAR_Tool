...
{
	string net_name = "tanh";
	string benchmark_name = "reachnn_benchmark_2_" + net_name;
	...
	*/
	...
	ODE<Real> dynamics({"x1 - x0^3","u","1","0"}, vars);
	...
	setting.setFixedStepsize(0.08, order);
	...
	double w = 0.1;
	int steps = 10;
	Interval init_x0(0.8 - w, 0.8 + w), init_x1(0.8 - w, 0.8 + w), init_t(0);
	Interval init_u(0);
	...
	string nn_name = "nn_2_"+net_name;
	NeuralNetwork nn(nn_name);

	// the order in use
	// unsigned int order = 5;
	Interval cutoff_threshold(-1e-12, 1e-12);
	unsigned int bernstein_order = 6;
	...
	Constraint c2("-x0 - 0.3", vars);		// x0 >= -0.3
	Constraint c3("x1 - 0.5", vars);		// x1 <= 0.5
	Constraint c4("-x1 - 0.35", vars);		// x1 >= -0.35
