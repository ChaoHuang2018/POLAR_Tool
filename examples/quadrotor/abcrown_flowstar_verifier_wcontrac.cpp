...
int main(int argc, char *argv[])
{
	...
	for (int iter = 0; iter < steps; ++iter)
	{
		...
		while(!initial_sets.empty()){
			...
			*/
			...
			string py_cmd_str = "python ./quadrotor/abcrown_flowstar_verifier_wcontrac.py --config ./quadrotor/quadrotor.yaml " + x_mins + x_maxs;
			const char* py_cmd = const_cast<char*>(py_cmd_str.c_str());
			system(py_cmd); 
			
			string py_tmp_str = "./abcrown_flowstar_tmp_wcontrac";
			...
			if(possible_argmax.size() < 1){
				...
				cout<<"Step" << iter << "stepFlowpipe will split into " << possible_argmax.size() << " new flowpipes." << endl;
		...
		while(!mid_initial_sets.empty()){
			...
			if(!b)
			{
				reach_result = "Flowpipe reaches target/unsafe set.";
				EXIT_FLG = true;
				cout << reach_result << endl;
				break;
			}
			else if (result.status == COMPLETED_SAFE || result.status == COMPLETED_UNSAFE || result.status == COMPLETED_UNKNOWN)
			{
	...
	const string dir_name =  "./outputs/abcrown_flowstar_quadrotor_crown_flowstar_wcontrac/";
	...
	ofstream result_output(dir_name + "/Steps_" + to_string(steps) + to_string(if_symbo)  + ".txt");
    ...
    plot_setting.plot_2D_octagon_GNUPLOT(c, "x1x4_" + to_string(steps) + "_steps_x_vx_" + to_string(if_symbo) , result);
	plot_setting.plot_2D_octagon_MATLAB(c, "x1x4_" + to_string(steps) + "_steps_x_vx_" + to_string(if_symbo) , result);

	plot_setting.setOutputDims("x2", "x5");
	plot_setting.plot_2D_octagon_GNUPLOT(c, "x2x5_" + to_string(steps) + "_steps_y_vy_" + to_string(if_symbo) , result);
	plot_setting.plot_2D_octagon_MATLAB(c, "x2x5_" + to_string(steps) + "_steps_y_vy_" + to_string(if_symbo) , result);
	
	plot_setting.setOutputDims("x3", "x6");
	plot_setting.plot_2D_octagon_GNUPLOT(c, "x3x6_" + to_string(steps) + "_steps_z_vz_" + to_string(if_symbo) , result);
	plot_setting.plot_2D_octagon_MATLAB(c, "x3x6_" + to_string(steps) + "_steps_z_vz_" + to_string(if_symbo) , result);

	plot_setting.setOutputDims("x1", "x2");
    plot_setting.plot_2D_octagon_GNUPLOT(c, "x1x2_" + to_string(steps) + "_steps_x_y_" + to_string(if_symbo) , result);
	plot_setting.plot_2D_octagon_MATLAB(c, "x1x2_" + to_string(steps) + "_steps_x_y_" + to_string(if_symbo) , result);
