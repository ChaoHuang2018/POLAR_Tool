...
int main(int argc, char *argv[])
{
	...
	*/
	...
	int x0_id = vars.declareVar("x0"); // x_pos
	int x1_id = vars.declareVar("x1");	// y_pos
	int x2_id = vars.declareVar("x2");	// x_vel
	int x3_id = vars.declareVar("x3");	// y_vel
	int x4_id = vars.declareVar("x4"); // ||v||
	int x5_id = vars.declareVar("x5");	// max_vel
	int x6_id = vars.declareVar("x6");	// norm_x_pos
	int x7_id = vars.declareVar("x7");	// norm_y_pos
	int x8_id = vars.declareVar("x8");	// norm_x_vel
	int x9_id = vars.declareVar("x9");	// norm_y_vel
 

	int u0_id = vars.declareVar("u0");	// fx_mean
	int u1_id = vars.declareVar("u1");	// fx_std	
	int u2_id = vars.declareVar("u2");	// fy_mean
	int u3_id = vars.declareVar("u3");	// fy_std

	int domainDim = numVars + 1;
	 
	// Define the continuous dynamics.
	Expression<Real> deriv_x0("x2", vars); //  deriv_x0 = (2 * x1 * deriv(x1) + 2 * x2 * deriv(x2))/2sqrt(x1*x1 + x2 * x2)
	Expression<Real> deriv_x1("x3", vars); //  deriv_x1 = x10 - x7
	Expression<Real> deriv_x2("2.0 * 0.001027 * x3 + 3 * 0.001027 * 0.001027 * x1 + u0 / 12.", vars); // deriv_x2 = x11 - x8
	Expression<Real> deriv_x3("-2.0 * 0.001027 * x2 + u1 / 12.", vars); // deriv_x3 = (2 * x4 * deriv(x4) + 2 * x5 * deriv(x5))/sqrt(x4*x4 + x5 * x5)
	Expression<Real> deriv_x4("((2.0 * 0.001027 * x3 + 3 * 0.001027 * 0.001027 * x1 + u0 / 12.) * x2 + (-2.0 * 0.001027 * x2 + u1 / 12.) * x3) / x4", vars); // deriv_x4 = x7
	Expression<Real> deriv_x5("2.0 * 0.001027 * (x0 * x2 + x1 * x3) / sqrt(x0 * x0 + x1 * x1)", vars); // deriv_x5 = x8
	Expression<Real> deriv_x6("x2 / 1000.0", vars); // deriv_x6 = (2 * x7 * deriv(x7) + 2 * x8 * deriv(x8))/sqrt(x7*x7 + x8 * x8)
	Expression<Real> deriv_x7("x3 / 1000.0", vars); // deriv_x7 = u3 * x7/x6 - u2 * x8
	Expression<Real> deriv_x8("(2.0 * 0.001027 * x3 + 3 * 0.001027 * 0.001027 * x1 + u0 / 12.) / 0.5", vars); // deriv_x8 = u3 * x8/x6 + u2 * x7
	Expression<Real> deriv_x9("(-2.0 * 0.001027 * x2 + u1 / 12.) / 0.5", vars); // deriv_x9 = (2 * x10 * deriv(x10) + 2 * x11 * deriv(x11))/sqrt(x10*x10 + x11 * x11)
	...
	Interval 
		init_x0(125), 
		init_x1(125), 
		init_x2((0.2 + 2.0 * 0.001027 * sqrt(125 * 125.0 + 125 * 125.0)) / sqrt(2)), 
		init_x3((0.2 + 2.0 * 0.001027 * sqrt(125 * 125.0 + 125 * 125.0)) / sqrt(2)), 
		init_x4((0.2 + 2.0 * 0.001027 * sqrt(125 * 125.0 + 125 * 125.0))), 
		init_x5((0.2 + 2.0 * 0.001027 * sqrt(125 * 125.0 + 125 * 125.0))), 
		init_x6(125 / 1000.0), 
		init_x7(125 / 1000.0),
		init_x8((0.2 + 2.0 * 0.001027 * sqrt(125 * 125.0 + 125 * 125.0)) / 0.5),
		init_x9((0.2 + 2.0 * 0.001027 * sqrt(125 * 125.0 + 125 * 125.0))),
	...
	*/
	...
	for (int iter = 0; iter < steps; ++iter)
	{
		...
		*/
		for (int i = 0; i < new int[]{6, 7, 8, 9, 4, 5}; i++)
		{
