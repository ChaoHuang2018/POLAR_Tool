...
### Model Dynamics
@taylorize function Benchmark1!(dx, x, p, t)
    dx[1] = x[2]
    dx[2] = (x[3]) * x[2]^2 - x[1]
    dx[3] = zero(x[3])
    return dx
...
const NAME = "nn_1_relu_tanh"
controller = read_nnet_polar(@modelpath("Benchmarks", "nn_1_relu_tanh"));

# controller_onnx = read_nnet_onnx(
#     ONNX.load(@modelpath("Benchmarks", "nn_1_sigmoid.onnx"),
#     zeros(Float32, 2)));

# @assert controller_onnx == controller

X₀ = Hyperrectangle(low=[0.8, 0.5],
                    high=[0.9, 0.6])
U₀ = ZeroSet(1);

vars_idx = Dict(:states=>1:2, :controls=>3)
ivp = @ivp(x' = Benchmark1!(x), dim: 3, x(0) ∈ X₀ × U₀)
...
print(io, "JuliaReach, Benchmark1, -, verified, $(res.time)\n")
