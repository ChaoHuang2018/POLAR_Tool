...
### Model Dynamics
@taylorize function benchmark5!(dx, x, p, t)
    dx[1] = x[3]^3 - x[2]
    dx[2] = x[3]
    dx[3] = x[4]
    dx[4] = zero(x[4])
...
controller = read_nnet_polar(@modelpath("Benchmarks", "nn_5_relu_tanh"));

X₀ = Hyperrectangle(low=[0.38, 0.45, 0.25],
                    high=[0.4, 0.47, 0.27])
U₀ = ZeroSet(1);

vars_idx = Dict(:states=>1:3, :controls=>4)
ivp = @ivp(x' = benchmark5!(x), dim: 4, x(0) ∈ X₀ × U₀)
...
target_states = cartesian_product(
    Hyperrectangle(low=[-0.43, 0.05, -100],
                   high=[-0.15, 0.22, 100]),
    Universe(1))
...
goal_states_x1x2 = Hyperrectangle(low=[-0.43, 0.05], high=[-0.15, 0.22])
...
alg = TMJets(abstol=1e-12, orderT=6, orderQ=1);
