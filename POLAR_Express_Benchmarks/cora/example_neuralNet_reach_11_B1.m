function completed = example_neuralNet_reach_11_B1
% example_neuralNet_reach_11_Benchmark 1 - example of reachability analysis for a
%                                   neural network controlled numerical
%                                   benchmark 1
%
%
% Syntax:
%    completed = example_neuralNet_reach_11_B1()
%
% Inputs:
%    no
%
...
options.timeStep = 0.1; %tanh 0.05 else 0.1
...
% open-loop system
f = @(x,u) [x(2); u(1)*x(2)*x(2) - x(1)];
sys = nonlinearSys(f);

% load neural network controller
% [2, 20, 20, 1]
nn = NeuralNetwork.readONNXNetwork('nn_1_relu_tanh.onnx');
...
safeSet = 1.2 * interval(-ones(2, 1), ones(2, 1));
...
xlim([-0.25, 1.2]);
ylim([-0.5, 0.6]);
rectangle('Position',[0 0.05 0.2 0.25])
