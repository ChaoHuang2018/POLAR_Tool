function completed = example_neuralNet_reach_13_B3
% example_neuralNet_reach_13_Benchmark 3 - example of reachability analysis for a
%                                   neural network controlled numerical
%                                   benchmark 3 in POLAR
%
%
% Syntax:
%    completed = example_neuralNet_reach_13_B3()
%
% Inputs:
%    no
%
...
disp("BENCHMARK3 from POLAR")

% Parameter ---------------------------------------------------------------
% initial set
R0 = interval([0.8;0.4],[0.9;0.5]);
% time horizon
params.tFinal = 6.0;
...
% open-loop system
f = @(x,u) [-x(1)*(0.1+(x(1)+x(2))^2); (u(1)+x(1))*(0.1+(x(1)+x(2))^2)];
sys = nonlinearSys(f);

% load neural network controller
nn = NeuralNetwork.readONNXNetwork('nn_3_relu_sigmoid.onnx');
...
xlim([0, 1]);
ylim([-0.4, 1]);
rectangle('Position',[0.2 -0.3 0.1 0.25])
